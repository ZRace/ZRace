            ...
            foreach (string prefabPath in prefabPaths)
            {
                ...
                try
                {
                    ...
                    if (!E_PlayerEvents.HasUnityEvent(target.GetComponent<vShooterWeapon>().onShot, "SendNetworkShot", target.GetComponent<MP_ShooterWeapon>()))
                    {
                        failed += 1;
                        failures.Add(new DebugFormat(
                            "vShooterWeapon - Missing the 'SendNetworkShot' UnityEvent on the 'OnShot' event. " +
                            "This means when this weapon is fired it will not be fired over the network.",
                            AssetDatabase.LoadMainAssetAtPath(prefabPath)
                        ));
                    }
                    else
                    ...
                    if (!E_PlayerEvents.HasUnityEvent(target.GetComponent<vShooterWeapon>().onEmptyClip, "SendNetworkEmptyClip", target.GetComponent<MP_ShooterWeapon>()))
                    {
                        failed += 1;
                        warnings.Add(new DebugFormat(
                            "vShooterWeapon - Missing the 'SendNetworkEmptyClip' UnityEvent on the 'onEmptyClip' event. " +
                            "This means when this weapon triggers an empty clip event it will not be triggered over the network.",
                            AssetDatabase.LoadMainAssetAtPath(prefabPath)
                        ));
                    }
                    else
