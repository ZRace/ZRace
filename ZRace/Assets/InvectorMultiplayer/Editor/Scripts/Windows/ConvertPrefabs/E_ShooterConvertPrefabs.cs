    ...
    public partial class E_ConvertPrefabs
    {
        ...
        static partial void CB_COMP_vShooterWeapon(GameObject target)
        {
            if (target.GetComponent<vShooterWeapon>())
            {
                ...
                if (!E_PlayerEvents.HasUnityEvent(target.GetComponent<vShooterWeapon>().onChangerPowerCharger, "SendOnChangerPowerCharger", target.GetComponent<MP_ShooterWeapon>()))
                {
                    UnityEventTools.AddPersistentListener(target.GetComponent<vShooterWeapon>().onChangerPowerCharger, target.GetComponent<MP_ShooterWeapon>().SendOnChangerPowerCharger);
                }
            }
            if (target.GetComponentInChildren<vShooterWeapon>())
            {
                foreach (vShooterWeapon weapon in target.GetComponentsInChildren<vShooterWeapon>())
                {
                    ...
                    if (!E_PlayerEvents.HasUnityEvent(weapon.onChangerPowerCharger, "SendOnChangerPowerCharger", weapon.gameObject.GetComponent<MP_ShooterWeapon>()))
                    {
                        UnityEventTools.AddPersistentListener(weapon.onChangerPowerCharger, weapon.gameObject.GetComponent<MP_ShooterWeapon>().SendOnChangerPowerCharger);
                    }
                }
            }
        }
