...
namespace HutongGames.PlayMaker.Pun2
{
    ...
    /// </summary>
    public class PlayMakerPhotonEditorUtility : UnityEditor.Editor
    {
        const string PlayMakerPhotonMenuRoot = "PlayMaker/Addons/Photon Networking/";

        public const float supportedPunVersion = 2.16f;
        ...
        */
        ...
        {
            string _punVersion = PhotonNetwork.PunVersion;

            string[] numbers = _punVersion.Split('.');
            if (numbers.Length > 2)
            {
                _punVersion = numbers[0] + "." + numbers[1];
            }

            float versionNumber = 0f;
            if (float.TryParse(_punVersion, out versionNumber))
            {
