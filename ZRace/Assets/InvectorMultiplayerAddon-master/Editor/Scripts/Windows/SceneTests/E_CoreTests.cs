    ...
    public partial class E_TestScene
    {
        ...
        void CORE_PerformSceneTransitionTests(ref int passed, ref int failed, ref List<DebugFormat> failures, ref List<DebugFormat> warnings)
        {
            ...
            if (!found)
            {
                failed += 1;
                failures.Add(new DebugFormat(
                    "SceneTransition - No \"SceneManager\" was found amount the root objects of this scene. " +
            ...
            else
            {
                ...
                foreach (GameObject point in GameObject.FindGameObjectsWithTag("LoadPoint"))
                {
                    ...
                    if (pointNames.Contains(point.name))
                    {
                        failed += 1;
                        warnings.Add(new DebugFormat(
                            "SceneTransition - This point name is name exactly the same thing as another entry point." +
                ...
                else
                {
                    ...
                    if ((bool)voiceChat.GetType().GetField("pushToTalk", E_Helpers.allBindings).GetValue(voiceChat) == true)
                    {
                        if (!allBindings.Contains((string)voiceChat.GetType().GetField("buttonToPress", E_Helpers.allBindings).GetValue(voiceChat)))
                        {
                            failed += 1;
                            failures.Add(new DebugFormat(
                                "VoiceChat - You have pushToTalk enabled but the specified button does not exist in your Project Settings > Input.",
                                voiceChat
                            ));
                        }
                        else
        ...
        void CORE_PerformSyncPlayerTests(ref int passed, ref int failed, ref List<DebugFormat> failures, ref List<DebugFormat> warnings)
        {
            ...
            foreach (SyncPlayer syncPlayer in syncPlayers)
            {
                ...
                else if ((float)syncPlayer.GetType().GetField("_positionLerpRate", bindFlags).GetValue(syncPlayer) > 100)
                {
                    passed += 1;
                    warnings.Add(new DebugFormat(
                        "SyncPlayer - Position Move Speed is above 100, this could result in 'snapping' movement, instead of smooth movement.",
                        syncPlayer.transform
                    ));
                }
                else
                ...
                if ((float)syncPlayer.GetType().GetField("_rotationLerpRate", bindFlags).GetValue(syncPlayer) > 100)
                {
                    passed += 1;
                    warnings.Add(new DebugFormat(
                        "SyncPlayer - Rotation Move Speed is above 100, this could result in 'snapping' movement, instead of smooth movement.",
                        syncPlayer.transform
                    ));
                }
                else
        ...
        void CORE_PerformNetworkManagerTests(ref int passed, ref int failed, ref List<DebugFormat> failures, ref List<DebugFormat> warnings)
        {
            ...
            else
            {
                ...
                BindingFlags bindFlags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static;
                FieldInfo field = nm.GetType().GetField("maxPlayerPerRoom", bindFlags);
                ...
                if (nm.autoSpawnPlayer == false)
                {
                    ...
                    warnings.Add(new DebugFormat(
                        ...
                        "to your first scene. NOTE: If using a Pre-Built UI, they have logic already completed to do this " +
                        "for you.",
                        nm
                    ));
                }
