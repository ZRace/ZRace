...
namespace CBGames.Editors
{
    public class E_ConvertAI : EditorWindow
    {
        ...
        #endregion
        [MenuItem("CB Games/Convert/(Alpha) AI Characters", false, 0)]
        ...
        public static void CB_COMP_AIvShooterWeapon(GameObject target)
        {
            if (target.GetComponent<vShooterWeapon>())
            {
                ...
                if (!E_PlayerEvents.HasUnityEvent(target.GetComponent<vShooterWeapon>().onChangerPowerCharger, "SendOnChangerPowerCharger", target.GetComponent<AI_MP_ShooterWeapon>()))
                {
                    UnityEventTools.AddPersistentListener(target.GetComponent<vShooterWeapon>().onChangerPowerCharger, target.GetComponent<AI_MP_ShooterWeapon>().SendOnChangerPowerCharger);
                }
            }
        }
        #endregion
