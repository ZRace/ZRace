        ...
        protected virtual void UpdateAmmoDisplay(int displayId)
        {
            ...
            if (useAmmoDisplay && ammoDisplay)
            {
                if (weapon.secundaryWeapon)
                {
                    ...
                    string display2 = weapon.secundaryWeapon.dontUseReload ? weapon.secundaryWeapon.isInfinityAmmo ? "Infinity" : (weapon.secundaryWeapon.ammoCount + secundaryTotalAmmo).ToString() : (string.Format("{0} | {1}", weapon.secundaryWeapon.ammoCount, (weapon.secundaryWeapon.isInfinityAmmo ? "Infinity" : secundaryTotalAmmo.ToString())));
                    ammoDisplay.UpdateDisplay(display1 + "\n" + display2, weapon.ammoID);
                }
                else
                {
